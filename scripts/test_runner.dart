...
class AutomatedTestPipeline {
  static const String _projectRoot =
      ...
      (Platform.environment['HOME'] ?? '/home/user') + '/termisol';
  ...
  Future<void> _runMemoryLeakTests() async {
    ...
    final testFiles = ['$_projectRoot/test/memory_leak_detector.dart'];
  ...
  Future<void> _runTestFile(String testFile, String category) async {
    ...
    if (result.exitCode == 0) {
      print(
        '  ✅ ${testFile.split('/').last} (${stopwatch.elapsedMilliseconds}ms)',
      );
...
<body>
  ...
  Future<void> _generateSecurityReport() async {
    ...
    final securityIssues = _results.issues
        .where((i) => i.type == 'security')
        .toList();
...
<body>
  ...
  void _printSummary(Stopwatch stopwatch) {
    ...
    print(
      '📈 Success Rate: ${(_results.successRate * 100).toStringAsFixed(1)}%',
    );
...
class TestResults {
  ...
  int get passedTests =>
      _categories.fold(0, (sum, cat) => sum + cat.passedTests);
  int get failedTests =>
      _categories.fold(0, (sum, cat) => sum + cat.failedTests);
  double get successRate => totalTests > 0 ? passedTests / totalTests : 0.0;
  int get totalDuration =>
      _categories.fold(0, (sum, cat) => sum + cat.totalDuration);
  ...
  List<PerformanceMetric> getPerformanceData() {
    ...
    for (final category in _categories) {
      if (category.name == 'performance') {
        metrics.add(
          PerformanceMetric('Test Duration', category.totalDuration, 'ms'),
        );
        metrics.add(
          PerformanceMetric('Tests Run', category.totalTests, 'count'),
        );
      }
    }
